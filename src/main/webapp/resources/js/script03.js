...
function updateSelectedCount() {
  ...
  const seatsIndex1 = [...selectedSeats].map((seat01) => [...seats].indexOf(seat01));
...
container.addEventListener('click', (event) => {
  ...
  if (event.target.classList.contains('seat01') && !event.target.classList.contains('occupied')) {
    event.target.classList.toggle('selected'); // 좌석 색깔이 파란색으로 변경됨
  }else if (event.target.classList.contains('seat02') && !event.target.classList.contains('occupied')) {
    event.target.classList.toggle('selected'); 
  }else if (event.target.classList.contains('seat03') && !event.target.classList.contains('occupied')) {
    event.target.classList.toggle('selected'); 
